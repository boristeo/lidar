...
"""
    ...
    dist_mm = data & 0x3fff# x | (( x1 & 0x3f) << 8) # distance is coded on 13 bits ? 14 bits ?
    quality = 0#x2 | (x3 << 8) # quality is on 16 bits
    ...
    if visualization:
        ...
        if not use_lines : lines[angle].pos[1]=(offset*c,offset*s,0)
        if not use_outer_line :
            outer_line.pos[angle]=(offset*c,offset*s,0)
            outer_line.color[angle] = (0.1, 0.1, 0.2)
        
        
        # display the sample
        if data & 0x8000: # is the flag for "bad data" set?
            # yes it's bad data
            lines[angle].pos[1]=(offset*c,offset*s, 0)
            outer_line.pos[angle]=(offset*c,offset*s, 0)
            outer_line.color[angle] = (0.1, 0.1, 0.2)
        else:
            # no, it's cool
            if not data & 0x4000:
                ...
                if use_points : pointb.pos[angle] = vector( dist_x, dist_y, 0)
                if use_intensity : point2b.pos[angle] = vector( (quality + offset)*c, (quality + offset)*s,0)
                if use_lines : lines[angle].color[1] = (0.4,0,0)
                if use_outer_line : outer_line.color[angle] = (0.4,0,0)
            if use_lines : lines[angle].pos[1]=( dist_x, dist_y,0)
            if use_outer_line : outer_line.pos[angle]=( dist_x, dist_y,0)
...
def read_Lidar():
    while True:
        try:
            ...
            angle = header[0]
            speed_rpm = header[1] / 64.0
            gui_update_speed(speed_rpm)
            count = header[2]
            print angle, speed_rpm, count
        ...
        try:
            ...
            if init_level == 0 :
                ...
                if checksum(all_data) == incoming_checksum:
                    ...
                    update_view(359 - index * 4, b_data0)
                    update_view(358 - index * 4, b_data1)
                    update_view(357 - index * 4, b_data2)
                    update_view(356 - index * 4, b_data3)
                else:
                    ...
                    update_view(359 - index * 4, [0, 0x80, 0, 0])
                    update_view(358 - index * 4, [0, 0x80, 0, 0])
                    update_view(357 - index * 4, [0, 0x80, 0, 0])
                    update_view(356 - index * 4, [0, 0x80, 0, 0])
